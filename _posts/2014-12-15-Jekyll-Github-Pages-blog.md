...
###前言###
  前段时间突然想给自己搭建一个blog，主要是想记录自己的所思所想所做，避免浑浑噩噩
  每一天（其实已经混了三年），我希望现在开始写还不晚。然后就开始买域名，整网站，忙活了两天才搞定。
  当然这并不一定得自己建个网站，事实上还有很多方式，而且更简单，比如弄个本子写日记，我有一个同学
  就一直坚持这样做。还有专门的blog平台，自己注册个账号就行。嫌麻烦，就用自己的新浪微博开通新浪博客服务，类似的服务很多。再不行？QQ空间里写日志 ^_^
...
##**一、Jekyll是什么？**##
  ...
  它就是一些文件和文件夹，你所做的只是本地编辑，然后用一个命令生成一个网站，也是一些文件和文件夹，
  最后用工具把这些文档上传到服务器就可以了。

  如果服务器支持Jekyll（比如GitHub Pages），你不必事先生成网站，
  直接用工具（比如Git）上传编辑好的文档目录就行，服务器端会自动生成网站，并且以后传入新的文档它也会自动更新。
  总之，使用Jekyll你只需要关心写作本身。

  Jekyll默认生成的网站样式可能有些简陋，如果不喜欢，你可以到[这里](http://jekyllthemes.org/)找自己喜欢的主题。
...
正如[Rei](http://chloerei.com/2013/09/30/back-to-jekyll/)所说，使用门槛很高，但除了第3点，也就是你打算用现成的主题话，
其他的只要用过一遍就会了，以后主要就是用Markdown写文本，Markdown值得学习。
...
### GitHub Pages的两种类型：###
  ...
  - **Project Pages网站**
      ...
      用作项目主页，展示关于一个项目的详细信息。个人/组织账户都可以创建。
      要创建一个Project Pages网站，只需在一个项目所在的仓库创建一个名为“gh-pages”的分支就可以了。
      gh-pages分支的内容将被自动托管，此时只要把创建好的网站文件放入该分支就可以了。
      它的访问URL默认是这样：
  ...
  - **User/Organization Pages网站**
      ...
      用作个人/组织主页，我的这个blog当然就是用的这种类型。其实也可用Project Pages，只不过是个约定习惯。
...
###3、为网站配置独立域名###
如果你自己有个域名，你也可以将网站指向到你独立域名下。事实上大家都这样干。
...
**GitHub支持配置两种类型的域名：**
  ...
  - 配置一个一级域名的方式
    ...
    有些域名注册商支持将一级域名指向到另一个域名，即类似为一级域名配置一个CNAME记录。
    但我在godaddy上注册的好像不行，只可以指向IP地址。
...
另外，以后你如果为自己的某个项目创建了Project GitHub page站点，比如是demo-app
现在通过[www/blog.]hooozer.com/demo-app就可以访问demo-app的主页了。
